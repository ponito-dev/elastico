...
class Field implements Arrayable
{
    ...
    public bool $time_series_dimension;

    public string $time_series_metric;
    ...
    /**
     * Deep-clone nested Field objects so a cloned field is fully independent
     * — getIndexProperties() hands out clones precisely because callers
     * mutate them (eachProperty callbacks), and a shallow copy would still
     * share its multi-fields and sub-properties with the original.
     */
    public function __clone()
    {
        if (isset($this->fields)) {
            $this->fields = array_map(
                static fn ($field) => $field instanceof self ? clone $field : $field,
                $this->fields,
            );
        }
        if (isset($this->properties)) {
            $this->properties = array_map(static fn (self $field): self => clone $field, $this->properties);
        }
    }
    ...
    /**
     * Multi-field mappings; accepts Field objects or raw config maps.
     *
     * @param array<int|string, Field|array> $fields
     */
    ...
    public function dimension(bool $dimension = true): static
    {
        $this->time_series_dimension = $dimension;
    ...
    {
        $this->time_series_metric = $metric;
    ...
    public function toArray(): array
    {
        ...
        if (isset($this->fields)) {
            $config['fields'] = collect($this->fields)
                ->mapWithKeys(fn ($field, $key) => $field instanceof self
                    ? [$field->getName() => $field->toArray()]
                    : [$key => $field])
                ->all();
        }
        if (isset($this->time_series_dimension)) {
            $config['time_series_dimension'] = $this->time_series_dimension;
        }
        if (isset($this->time_series_metric)) {
            $config['time_series_metric'] = $this->time_series_metric;
        }
