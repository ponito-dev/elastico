...
class Builder extends BaseBuilder
{
    ...
    public $ignore_unavailable = false;
    ...
    {
        return $this->onceWithColumns(Arr::wrap($columns), function () use ($id) {
            return $this->processor->processFind(
                $this,
                $this->connection->find(
                    // Columns come from the query, not the parameter:
                    // onceWithColumns keeps an earlier select() and only
                    // falls back to the parameter when none was made, so
                    // reading the parameter here discarded every select().
                    $this->grammar->compileGet($this->from, $id, $this->columns)
                )
            );
        });
    }
    ...
    {
        return $this->onceWithColumns(Arr::wrap($columns), function () use ($ids) {
            return $this->processor->processFindMany(
                $this,
                $this->connection->findMany(
                    $this->grammar->compileFindMany($this->from, $ids, $this->columns)
                )
            );
        });
    }
     ...
     */
    public function cursor($keepAlive = '1m', ?int $pageSize = null)
    {
        if (is_null($this->columns)) {
            $this->columns = ['*'];
        }

        return new LazyCollection(function () use ($keepAlive, $pageSize) {
    ...
    {
        throw new RuntimeException('insertOrIgnore() is not supported by the Elasticsearch grammar; use insert() with ignoreConflicts().');
    }
    ...
    {
        throw new RuntimeException('insertGetId() is not supported by the Elasticsearch grammar; use insert() and read the generated _id from the bulk response items.');
    }
    ...
    {
        throw new RuntimeException('insertUsing() is not supported by the Elasticsearch grammar.');
    }
    ...
    public function delete($id = null)
    {
        ...
        if (!is_null($id)) {
            return $this->connection->deleteDocument($id, $this->from);
    ...
    public function exists()
    {
        ...
        $results = $this->processor->resolvePromise($this->connection->select(
            $this->grammar->compileExists($this),
            $this->getBindings(),
            !$this->useWritePdo
        ));

        return ($results['hits']['total']['value'] ?? 0) > 0;
    }
    ...
    /**
     * Treat missing indices as empty instead of erroring.
     */
    public function ignoreUnavailable(bool $ignore = true): static
    {
        $this->ignore_unavailable = $ignore;
