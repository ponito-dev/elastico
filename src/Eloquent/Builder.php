...
class Builder extends EloquentBuilder
{
    ...
    public function find($id, $columns = ['*'])
    {
        ...
        // The document GET api cannot target a data stream name; stream
        // documents resolve through a search on the id instead. Global
        // scopes (SoftDeletes included) cannot be expressed on a GET
        // either, so scoped queries take the search path too.
        if ($this->model instanceof DataStream || !empty($this->scopes)) {
            return $this->whereKey($id)->first($columns);
        }

        $result = $this->toBase()->find($id, $columns);

        if (is_null($result)) {
            return null;
        }

        return $this->model->hydrate([$result])->first();
    }
    ...
    public function findMany($ids, $columns = ['*'])
    {
        ...
        // The multi-GET api cannot target a data stream name either, and
        // cannot apply global scopes.
        if ($this->model instanceof DataStream || !empty($this->scopes)) {
    ...
    {
        // applyScopes() returns a scoped clone; the clone's query — not
        // $this->query — must be the one executed, or global scopes
        // (SoftDeletes included) silently drop off the request.
        $builder = $this->applyScopes();

        $builder->passColumnNamesToQuery($columns);

        $response = $builder->query->get($columns);

        $models = $builder->model->hydrate($response->all())->all();
        ...
        return $builder->getQuery()->applyAfterQueryCallbacks($response->resetItems($models));
    }
    ...
    public function getModels($columns = ['*'])
    {
        ...
        return $builder->model->hydrate(
            $builder->query->get($columns)->all()
        )->all();
    }
    ...
    {
        $pageSize = (int) max(1, min($pageSize ?? static::DEFAULT_CHUNK_SIZE, Connection::MAX_CURSOR_SIZE));

        if (empty($this->query->columns) || $this->query->columns === ['*']) {
            $this->select($this->getModel()->getFieldNames());
        }
        return $this->applyScopes()->query->cursor(keepAlive: $keepAlive, pageSize: $pageSize)
